...
export class ProdPlanService {
	constructor(@InjectRepository(Prodplan001hb) private readonly prodPlanhbRepository: Repository<Prodplan001hb>,
				@InjectRepository(Prodplan001mb) private readonly prodPlanRepository: Repository<Prodplan001mb>,) { }
	...
	async remove(id: number): Promise<void> {
        let prodplan = this.prodPlanRepository.findOne(id);
        let prodplan001hb = new Prodplan001hb();
        prodplan001hb.setProperties((await prodplan));
        this.prodPlanhbRepository.save(prodplan001hb);
        await this.prodPlanRepository.delete(id);
    }

	async downloadPdf(@Req() request: Request, @Res() response: Response) {
		var html = fs.readFileSync("./html_pdf/proPlan.html", "utf8");
