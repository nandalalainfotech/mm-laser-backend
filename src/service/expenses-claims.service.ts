...
export class ExpensesClaimsService {
    constructor(@InjectRepository(Expensesclm001mb) private readonly expensesclmRepository: Repository<Expensesclm001mb>,
        @InjectRepository(Expensesclm001hb) private readonly expensesclmhbRepository: Repository<Expensesclm001hb>) { }
    ...
    async remove(id: string): Promise<void> {
        let expensesclm = this.expensesclmRepository.findOne(id);
        let expensesclm001hb = new Expensesclm001hb();
        expensesclm001hb.setProperties((await expensesclm));
        this.expensesclmhbRepository.save(expensesclm001hb);
        await this.expensesclmRepository.delete(id);
    }

    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/expensesclm.html", "utf8");
