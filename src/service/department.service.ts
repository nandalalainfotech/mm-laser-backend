...
export class DepartmentService {
    ...
    constructor(@InjectRepository(Department001mb) private readonly departmentRepository: Repository<Department001mb>,
        @InjectRepository(Department001hb) private readonly departmenthbRepository: Repository<Department001hb>) { }
    ...
    async update(departmentDTO: DepartmentDTO): Promise<Department001mb> {
        const department001mb = new Department001mb();
        department001mb.setProperties(departmentDTO);
        await this.departmentRepository.update({ deptid: department001mb.deptid }, department001mb);
    ...
    findOne(id: string): Promise<Department001mb> {
        return this.departmentRepository.findOne(id);
    }

    async remove(id: string): Promise<void> {
        let department = this.departmentRepository.findOne(id);
        let department001hb = new Department001hb();
        department001hb.setProperties((await department));
        this.departmenthbRepository.save(department001hb);
        await this.departmentRepository.delete(id);
    }

    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/department.html", "utf8");
        let department: any[] = await this.departmentRepository.find();
    ...
    async downloadExcel(@Req() request: Request, @Res() response: Response) {
        ...
        let department = await this.departmentRepository.find();
        ...
        if (department.length < 0) {
            ...
            worksheet.getCell('A2:B2').value = "Add New Department";
