...
export class RelievingLetterService {
    constructor(@InjectRepository(Recruitrelieving001hb) private readonly relievingLetterhbRepository: Repository<Recruitrelieving001hb>,
                @InjectRepository(Recruitrelieving001mb) private readonly relievingLetterRepository: Repository<Recruitrelieving001mb>) { }
    ...
    async remove(id: number): Promise<void> {
        let recruitrelieving = this.relievingLetterRepository.findOne(id);
        let recruitrelieving001hb = new Recruitrelieving001hb();
        recruitrelieving001hb.setProperties((await recruitrelieving));
        this.relievingLetterhbRepository.save(recruitrelieving001hb);
        await this.relievingLetterRepository.delete(id);
    }
    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/relievingLetter.html", "utf8");
