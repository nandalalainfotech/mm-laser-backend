...
export class CrmRepMiniTimeService {
    constructor(@InjectRepository(Crmmintoresp001mb) private readonly minitimeRepository: Repository<Crmmintoresp001mb>,
                @InjectRepository(Crmmintoresp001hb) private readonly minitimehbRepository: Repository<Crmmintoresp001hb>,) { }
    ...
    async remove(id: string): Promise<void> {
        let Crmmintoresp = this.minitimeRepository.findOne(id);
        let crmmintoresp001hb = new Crmmintoresp001hb();
        crmmintoresp001hb.setProperties((await Crmmintoresp));
        this.minitimehbRepository.save(crmmintoresp001hb);
        await this.minitimeRepository.delete(id);
    }

    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/minitime.html", "utf8");
