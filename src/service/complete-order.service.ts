...
export class CompleteOrderService {
    constructor(@InjectRepository(Compprodorder001mb) private readonly compprodorderRepository: Repository<Compprodorder001mb>,
        @InjectRepository(Compprodorder001hb) private readonly compprodorderhbRepository: Repository<Compprodorder001hb>) { }
    ...
    async remove(id: string): Promise<void> {
        let Compprodorder = this.compprodorderRepository.findOne(id);
        let compprodorder001hb = new Compprodorder001hb();
        compprodorder001hb.setProperties((await Compprodorder));
        this.compprodorderhbRepository.save(compprodorder001hb);
        await this.compprodorderRepository.delete(id);
    }

    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/complete-order.html", "utf8");
        let completeOrder: any[] = await this.compprodorderRepository.find();
        ...
        var document = {
            ...
            path: path.join(`./pdf/completeOrder.pdf`),
            type: "",
        };
        var options = {
            format: "A3",
        ...
        if (completeOrder.length < 0) {
            ...
            pdf.create(document, options).then((pathRes) => {
                ...
                response.writeHead(200, {
                    "Content-Disposition": "attachment;filename=" + "completeOrder.pdf",
    ...
    async downloadExcel(@Req() request: Request, @Res() response: Response) {
        ...
        let completeOrder = await this.compprodorderRepository.find();
        ...
        if (completeOrder.length < 0) {
            ...
            let worksheet = workbook.addWorksheet('completeOrder');
            ...
            worksheet.getCell('A2:G2').value = "Complete Production Order";
            ...
            for (let i = 0; i < completeOrder.length; i++) {
                ...
                worksheet.getCell('A' + temp).value = completeOrder[i].prodId;
                ...
                worksheet.getCell('B' + temp).value = completeOrder[i].prorderCode;
                ...
                worksheet.getCell('C' + temp).value = completeOrder[i].date;
                ...
                worksheet.getCell('D' + temp).value = completeOrder[i].itemtoManufacture;
                ...
                worksheet.getCell('E' + temp).value = completeOrder[i].toProduce;
                ...
                worksheet.getCell('F' + temp).value = completeOrder[i].produced;
                ...
                worksheet.getCell('G' + temp).value = completeOrder[i].empCompany;
