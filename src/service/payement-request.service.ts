...
export class PayementRequestService {
	constructor(@InjectRepository(Paymentrequest001hb) private readonly payementrequesthbRepository: Repository<Paymentrequest001hb>,
                @InjectRepository(Paymentrequest001mb) private readonly payementrequestRepository: Repository<Paymentrequest001mb>) { }
    ...
    async remove(id: number): Promise<void> {
        let paymentrequest = this.payementrequestRepository.findOne(id);
        let paymentrequest001hb = new Paymentrequest001hb();
        paymentrequest001hb.setProperties((await paymentrequest));
        this.payementrequesthbRepository.save(paymentrequest001hb);
        await this.payementrequestRepository.delete(id);
    }

	async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/payementrequest.html", "utf8");
