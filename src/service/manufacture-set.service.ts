...
export class ManufactureSetService {
  constructor(
    @InjectRepository(Manufactureset001mb)
    private readonly manufacturesetRepository: Repository<Manufactureset001mb>,
    @InjectRepository(Manufactureset001hb)
    private readonly manufacturesethbRepository: Repository<Manufactureset001hb>
  ) {}

  async create(
    manufacturesetDTO: ManufacturesetDTO
  ): Promise<Manufactureset001mb> {
  ...
  async update(
    ...
    const manufactureset001mb = new Manufactureset001mb();
    manufactureset001mb.setProperties(manufacturesetDTO);
    await this.manufacturesetRepository.update(
      { mansetId: manufactureset001mb.mansetId },
      manufactureset001mb
    );
  ...
  async remove(id: string): Promise<void> {
    let Manufactureset = this.manufacturesetRepository.findOne(id);
    let manufactureset001hb = new Manufactureset001hb();
    manufactureset001hb.setProperties(await Manufactureset);
    this.manufacturesethbRepository.save(manufactureset001hb);
    await this.manufacturesetRepository.delete(id);
  }

  async downloadPdf(@Req() request: Request, @Res() response: Response) {
    var html = fs.readFileSync("./html_pdf/manu-setup.html", "utf8");
