...
export class PayementEntryService {
	constructor(@InjectRepository(Paymententry001hb) private readonly payementhbRepository: Repository<Paymententry001hb>,
				@InjectRepository(Paymententry001mb) private readonly payementRepository: Repository<Paymententry001mb>) { }
	...
	async remove(id: number): Promise<void> {
        let paymententry = this.payementRepository.findOne(id);
        let paymententry001hb = new Paymententry001hb();
        paymententry001hb.setProperties((await paymententry));
        this.payementhbRepository.save(paymententry001hb);
        await this.payementRepository.delete(id);
    }

	async downloadPdf(@Req() request: Request, @Res() response: Response) {
		var html = fs.readFileSync("./html_pdf/payementEntry.html", "utf8");
