...
export class BudgetAccountService {
    ...
    constructor(@InjectRepository(Budgetaccounttype001mb) private readonly budgetAccountRepository: Repository<Budgetaccounttype001mb>,
        @InjectRepository(Budgetaccounttype001hb) private readonly budgetAccounthbRepository: Repository<Budgetaccounttype001hb>) { }
    ...
    async remove(id: string): Promise<void> {
        let budgetAccount = this.budgetAccountRepository.findOne(id);
        let budgetaccounttype001hb = new Budgetaccounttype001hb();
        budgetaccounttype001hb.setProperties((await budgetAccount));
        this.budgetAccounthbRepository.save(budgetaccounttype001hb);
        await this.budgetAccountRepository.delete(id);
    }

    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/budgetAccount.html", "utf8");
