...
export class ItemGroupService {
  constructor(
    @InjectRepository(Itemgroup001mb)
    private readonly itemGroupRepository: Repository<Itemgroup001mb>,
    @InjectRepository(Itemgroup001hb)
    private readonly itemGrouphbRepository: Repository<Itemgroup001hb>
  ) {}
  ...
  async update(itemgroupDTO: ItemgroupDTO): Promise<Itemgroup001mb> {
    const itemgroup001mb = new Itemgroup001mb();
    itemgroup001mb.setProperties(itemgroupDTO);
    await this.itemGroupRepository.update(
      { igId: itemgroup001mb.igId },
      itemgroup001mb
    );
  ...
  async remove(id: string): Promise<void> {
    let Itemgroup = this.itemGroupRepository.findOne(id);
    let itemgroup001hb = new Itemgroup001hb();
    itemgroup001hb.setProperties(await Itemgroup);
    this.itemGrouphbRepository.save(itemgroup001hb);
