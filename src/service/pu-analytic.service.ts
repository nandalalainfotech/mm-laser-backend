...
export class PuAnalyticService {
    ...
    constructor (@InjectRepository(Puranalytics001hb) private readonly puAnalytichbRepository: Repository<Puranalytics001hb>,
                 @InjectRepository(Puranalytics001mb) private readonly puAnalyticRepository: Repository<Puranalytics001mb>) {}
    ...
    async remove(id: number): Promise<void> {
        let puranalytics = this.puAnalyticRepository.findOne(id);
        let puranalytics001hb = new Puranalytics001hb();
        puranalytics001hb.setProperties((await puranalytics));
        this.puAnalytichbRepository.save(puranalytics001hb);
        await this.puAnalyticRepository.delete(id);
    }

    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/pu-analytic.html", "utf8");
