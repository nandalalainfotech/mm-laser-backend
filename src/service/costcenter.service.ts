...
export class CostCenterService {
    ...
    constructor(@InjectRepository(Costcenter001mb) private readonly costCenterRepository: Repository<Costcenter001mb>,
        @InjectRepository(Costcenter001hb) private readonly costCenterhbRepository: Repository<Costcenter001hb>,) { }
    ...
    async remove(id: string): Promise<void> {
        let Costcenter = this.costCenterRepository.findOne(id);
        let costcenter001hb = new Costcenter001hb();
        costcenter001hb.setProperties((await Costcenter));
        this.costCenterhbRepository.save(costcenter001hb);
        await this.costCenterRepository.delete(id);
    }

    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/costCenter.html", "utf8");
