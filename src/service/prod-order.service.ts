...
export class ProdOrderService {
	constructor(@InjectRepository(Prodorder001hb) private readonly prodorderhbRepository: Repository<Prodorder001hb>,
		@InjectRepository(Prodorder001mb) private readonly prodorderRepository: Repository<Prodorder001mb>,) { }
	...
	async remove(id: number): Promise<void> {
		let prodorder = this.prodorderRepository.findOne(id);
		let prodorder001hb = new Prodorder001hb();
		prodorder001hb.setProperties((await prodorder));
		this.prodorderhbRepository.save(prodorder001hb);
		await this.prodorderRepository.delete(id);
	}

	async downloadPdf(@Req() request: Request, @Res() response: Response) {
		var html = fs.readFileSync("./html_pdf/proOrder.html", "utf8");
