...
export class EmployeeSalesRegisterService {
    constructor(@InjectRepository(Empsalaryregister001mb) private readonly employeeSalesRegisterRepository: Repository<Empsalaryregister001mb>,
        @InjectRepository(Empsalaryregister001hb) private readonly employeeSalesRegisterhbRepository: Repository<Empsalaryregister001hb>) { }
    ...
    async remove(id: number): Promise<void> {
        let employeeSalesRegister = this.employeeSalesRegisterRepository.findOne(id);
        let empsalaryregister001hb = new Empsalaryregister001hb();
        empsalaryregister001hb.setProperties((await employeeSalesRegister));
        this.employeeSalesRegisterhbRepository.save(empsalaryregister001hb);
        await this.employeeSalesRegisterRepository.delete(id);
    }

    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/employeeSalesRegister.html", "utf8");
