...
export class HrProcessPrService {
  constructor(
    @InjectRepository(Processpr001mb)
    private readonly processprRepository: Repository<Processpr001mb>,
    @InjectRepository(Processpr001hb)
    private readonly processprhbRepository: Repository<Processpr001hb>
  ) {}
  ...
  async update(processprDTO: ProcessprDTO): Promise<Processpr001mb> {
    const processpr001mb = new Processpr001mb();
    processpr001mb.setProperties(processprDTO);
    await this.processprRepository.update(
      { processId: processpr001mb.processId },
      processpr001mb
    );
  ...
  async remove(id: string): Promise<void> {
    let processpr = this.processprRepository.findOne(id);
    let processpr001hb = new Processpr001hb();
    processpr001hb.setProperties(await processpr);
    this.processprhbRepository.save(processpr001hb);
    await this.processprRepository.delete(id);
  }

  async downloadPdf(@Req() request: Request, @Res() response: Response) {
    var html = fs.readFileSync("./html_pdf/processpr.html", "utf8");
  ...
  async downloadExcel(@Req() request: Request, @Res() response: Response) {
    ...
    if (processpr.length < 0) {
      ...
      worksheet.getCell("A1:F1").alignment = {
        vertical: "middle",
        horizontal: "center",
      };
