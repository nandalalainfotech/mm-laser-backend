...
export class SuppTypeService {
	constructor(@InjectRepository(Supptype001hb) private readonly supptypehbRepository: Repository<Supptype001hb>,
                @InjectRepository(Supptype001mb) private readonly supptypeRepository: Repository<Supptype001mb>,) { }
    ...
    async remove(id: number): Promise<void> {
        let supptype = this.supptypeRepository.findOne(id);
        let supptype001hb = new Supptype001hb();
        supptype001hb.setProperties((await supptype));
        this.supptypehbRepository.save(supptype001hb);
        await this.supptypeRepository.delete(id);
    }

	async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/suppType.html", "utf8");
