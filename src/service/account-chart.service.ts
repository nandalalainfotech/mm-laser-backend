...
export class AccountChartService {
    constructor(@InjectRepository(Accountchart001mb) private readonly accChartRepository: Repository<Accountchart001mb>,
        @InjectRepository(Accountchart001hb) private readonly accCharthbRepository: Repository<Accountchart001hb>,) { }
    ...
    async remove(id: string): Promise<void> {
        let accountchart = this.accChartRepository.findOne(id);
        let accountchart001hb = new Accountchart001hb();
        accountchart001hb.setProperties((await accountchart));
        this.accCharthbRepository.save(accountchart001hb);
        await this.accChartRepository.delete(id);
    }

    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/accChart.html", "utf8");
