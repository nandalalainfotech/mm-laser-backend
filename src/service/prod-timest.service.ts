...
export class ProdTimeSheetService {
	constructor(@InjectRepository(Prodtimesheet001hb) private readonly prodTimeSheethbRepository: Repository<Prodtimesheet001hb>,
				@InjectRepository(Prodtimesheet001mb) private readonly prodTimeSheetRepository: Repository<Prodtimesheet001mb>,) { }
	...
	async remove(id: number): Promise<void> {
        let prodtimesheet = this.prodTimeSheetRepository.findOne(id);
        let prodtimesheet001hb = new Prodtimesheet001hb();
        prodtimesheet001hb.setProperties((await prodtimesheet));
        this.prodTimeSheethbRepository.save(prodtimesheet001hb);
        await this.prodTimeSheetRepository.delete(id);
    }

	async downloadPdf(@Req() request: Request, @Res() response: Response) {
		var html = fs.readFileSync("./html_pdf/prodTimest.html", "utf8");
