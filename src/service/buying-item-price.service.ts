...
export class BuyingItemPriceService {
    constructor(@InjectRepository(Itemprice001mb) private readonly itemPriceRepository: Repository<Itemprice001mb>,
        @InjectRepository(Itemprice001hb) private readonly itemPricehbRepository: Repository<Itemprice001hb>,) { }
    ...
    async remove(id: string): Promise<void> {
        let itemPrice = this.itemPriceRepository.findOne(id);
        let itemprice001hb = new Itemprice001hb();
        itemprice001hb.setProperties((await itemPrice));
        this.itemPricehbRepository.save(itemprice001hb);
        await this.itemPriceRepository.delete(id);
    }

    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/buying-itemPrice.html", "utf8");
