...
export class ProjectQuantityService {
	constructor(@InjectRepository(Stkrepproject001hb) private readonly prjoctQtyhbRepository: Repository<Stkrepproject001hb>,
                @InjectRepository(Stkrepproject001mb) private readonly prjoctQtyRepository: Repository<Stkrepproject001mb>,) { }
	...
	async remove(id: number): Promise<void> {
        let stkrepproject = this.prjoctQtyRepository.findOne(id);
        let stkrepproject001hb = new Stkrepproject001hb();
        stkrepproject001hb.setProperties((await stkrepproject));
        this.prjoctQtyhbRepository.save(stkrepproject001hb);
        await this.prjoctQtyRepository.delete(id);
    }

	
    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/prjoctQty.html", "utf8");
