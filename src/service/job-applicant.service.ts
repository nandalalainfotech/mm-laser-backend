...
export class JobApplicantService {
  constructor(
    @InjectRepository(Recruitapplicant001mb)
    private readonly jobApplicantRepository: Repository<Recruitapplicant001mb>,
    @InjectRepository(Recruitapplicant001hb)
    private readonly jobApplicanthbRepository: Repository<Recruitapplicant001hb>
  ) {}

  async create(
    recruitapplicantDTO: RecruitapplicantDTO
  ): Promise<Recruitapplicant001mb> {
  ...
  async update(
    ...
    const recruitapplicant001mb = new Recruitapplicant001mb();
    recruitapplicant001mb.setProperties(recruitapplicantDTO);
    await this.jobApplicantRepository.update(
      { appId: recruitapplicant001mb.appId },
      recruitapplicant001mb
    );
  ...
  async remove(id: number): Promise<void> {
    let Recruitapplicant = this.jobApplicantRepository.findOne(id);
    let recruitapplicant001hb = new Recruitapplicant001hb();
    recruitapplicant001hb.setProperties(await Recruitapplicant);
    this.jobApplicanthbRepository.save(recruitapplicant001hb);
    await this.jobApplicantRepository.delete(id);
  }
  async downloadPdf(@Req() request: Request, @Res() response: Response) {
    var html = fs.readFileSync("./html_pdf/jobApplicant.html", "utf8");
