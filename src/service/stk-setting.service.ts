...
export class StockSettingService {
    constructor(@InjectRepository(Stksettings001hb) private readonly stockSettinghbRepository: Repository<Stksettings001hb>,
                @InjectRepository(Stksettings001mb) private readonly stockSettingRepository: Repository<Stksettings001mb>) { }
    ...
    async remove(id: number): Promise<void> {
        let stksettings = this.stockSettingRepository.findOne(id);
        let stksettings001hb = new Stksettings001hb();
        stksettings001hb.setProperties((await stksettings));
        this.stockSettinghbRepository.save(stksettings001hb);
        await this.stockSettingRepository.delete(id);
    }

    
    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/stockSetting.html", "utf8");
