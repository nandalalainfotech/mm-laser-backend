...
export class EmpAttendanceService {
    constructor(@InjectRepository(Empattendance001mb) private readonly EmpAttendanceRepository: Repository<Empattendance001mb>,
        @InjectRepository(Empattendance001hb) private readonly EmpAttendancehbRepository: Repository<Empattendance001hb>) { }
    ...
    async remove(id: string): Promise<void> {
        let EmpAttendance = this.EmpAttendanceRepository.findOne(id);
        let empattendance001hb = new Empattendance001hb();
        empattendance001hb.setProperties((await EmpAttendance));
        this.EmpAttendancehbRepository.save(empattendance001hb);
        await this.EmpAttendanceRepository.delete(id);
    }
    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/EmpAttendance.html", "utf8");
