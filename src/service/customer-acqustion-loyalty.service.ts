...
export class CustomerLoyaltyService {
    constructor(@InjectRepository(Customeracquisition001mb) private readonly customerLoyaltyRepository: Repository<Customeracquisition001mb>,
        @InjectRepository(Customeracquisition001hb) private readonly customerLoyaltyhbRepository: Repository<Customeracquisition001hb>,) { }
    ...
    async remove(id: string): Promise<void> {
        let Customeracquisition = this.customerLoyaltyRepository.findOne(id);
        let customeracquisition001hb = new Customeracquisition001hb();
        customeracquisition001hb.setProperties((await Customeracquisition));
        this.customerLoyaltyhbRepository.save(customeracquisition001hb);
        await this.customerLoyaltyRepository.delete(id);
    }


    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/customerLoyalty.html", "utf8");
