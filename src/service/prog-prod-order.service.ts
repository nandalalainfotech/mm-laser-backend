...
export class ProgProdOrderService {
     constructor(@InjectRepository(Progprodorder001hb) private readonly progProdOrderhbRepository: Repository<Progprodorder001hb>,
                 @InjectRepository(Progprodorder001mb) private readonly progProdOrderRepository: Repository<Progprodorder001mb>) { }
    ...
    async remove(id: number): Promise<void> {
        let progprodorder = this.progProdOrderRepository.findOne(id);
        let progprodorder001hb = new Progprodorder001hb();
        progprodorder001hb.setProperties((await progprodorder));
        this.progProdOrderhbRepository.save(progprodorder001hb);
        await this.progProdOrderRepository.delete(id);
    }

    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/prog-order.html", "utf8");
