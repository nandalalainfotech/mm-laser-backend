...
export class SalesInvoiceService {
    constructor(@InjectRepository(Grossprofit001hb) private readonly salesinvoicehbRepository: Repository<Grossprofit001hb>,
                @InjectRepository(Grossprofit001mb) private readonly salesinvoiceRepository: Repository<Grossprofit001mb>,) { }
    ...
    async remove(id: number): Promise<void> {
        let grossprofit = this.salesinvoiceRepository.findOne(id);
        let grossprofit001hb = new Grossprofit001hb();
        grossprofit001hb.setProperties((await grossprofit));
        this.salesinvoicehbRepository.save(grossprofit001hb);
        await this.salesinvoiceRepository.delete(id);
    }

    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/salesinvoice.html", "utf8");
