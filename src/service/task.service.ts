...
export class TaskService {
    ...
    constructor (@InjectRepository(Task001mb) private readonly taskRepository: Repository<Task001mb>,
                 @InjectRepository(Task001hb) private readonly taskhbRepository: Repository<Task001hb>) {}
    ...
    async remove(id: string): Promise<void> {
        ...
        let task001hb = new Task001hb();
        task001hb.setProperties((await task));
        this.taskhbRepository.save(task001hb);
        await this.taskRepository.delete(id);
    }


    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/task.html", "utf8");
