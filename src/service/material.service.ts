...
export class Materialservice {
  constructor(
    ...
    private readonly materialRepository: Repository<Itemdt001mb>,
    @InjectRepository(Itemdt001hb)
    private readonly materialhbRepository: Repository<Itemdt001hb>
  ) {}
  ...
  async remove(id: string): Promise<void> {
    let Itemdt = this.materialRepository.findOne(id);
    let itemdt001hb = new Itemdt001hb();
    itemdt001hb.setProperties(await Itemdt);
    this.materialhbRepository.save(itemdt001hb);
    await this.materialRepository.delete(id);
  }

  async downloadPdf(@Req() request: Request, @Res() response: Response) {
    var html = fs.readFileSync("./html_pdf/material.html", "utf8");
