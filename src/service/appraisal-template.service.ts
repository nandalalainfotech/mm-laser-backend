...
export class AppraisalTemplateService {
    ...
    constructor(@InjectRepository(Apprtemp001mb) private readonly appraisaltempRepository: Repository<Apprtemp001mb>,
        @InjectRepository(Apprtemp001hb) private readonly appraisaltemphbRepository: Repository<Apprtemp001hb>) { }
    ...
    async remove(id: number): Promise<void> {
        let applicationsetting = this.appraisaltempRepository.findOne(id);
        let apprtemp001hb = new Apprtemp001hb();
        apprtemp001hb.setProperties((await applicationsetting));
        this.appraisaltemphbRepository.save(apprtemp001hb);
        await this.appraisaltempRepository.delete(id);
    }

    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/appraisalTemplate.html", "utf8");
