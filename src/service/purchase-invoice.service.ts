...
export class PurchaseInvoiceService {
    constructor(@InjectRepository(Grossprofit001hb) private readonly purchaseinvoicehbRepository: Repository<Grossprofit001hb>,
                @InjectRepository(Grossprofit001mb) private readonly purchaseinvoiceRepository: Repository<Grossprofit001mb>,) { }
    ...
    async remove(id: number): Promise<void> {
        let grossprofit = this.purchaseinvoiceRepository.findOne(id);
        let grossprofit001hb = new Grossprofit001hb();
        grossprofit001hb.setProperties((await grossprofit));
        this.purchaseinvoicehbRepository.save(grossprofit001hb);
        await this.purchaseinvoiceRepository.delete(id);
    }

    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/purchaseinvoice.html", "utf8");
