...
export class SupStatusService {
	constructor(@InjectRepository(Supstatus001hb) private readonly supstatushbRepository: Repository<Supstatus001hb>,
                @InjectRepository(Supstatus001mb) private readonly supstatusRepository: Repository<Supstatus001mb>,) { }
    ...
    async remove(id: number): Promise<void> {
        let supstatus = this.supstatusRepository.findOne(id);
        let supstatus001hb = new Supstatus001hb();
        supstatus001hb.setProperties((await supstatus));
        this.supstatushbRepository.save(supstatus001hb);
        await this.supstatusRepository.delete(id);
    }

	async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/supStatus.html", "utf8");
