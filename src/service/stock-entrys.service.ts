...
export class StockEntryService {
	constructor(@InjectRepository(Stockentry001hb) private readonly stockEntryhbRepository: Repository<Stockentry001hb>,
                @InjectRepository(Stockentry001mb) private readonly stockEntryRepository: Repository<Stockentry001mb>,) { }
	...
	async remove(id: number): Promise<void> {
        let stockentry = this.stockEntryRepository.findOne(id);
        let stockentry001hb = new Stockentry001hb();
        stockentry001hb.setProperties((await stockentry));
        this.stockEntryhbRepository.save(stockentry001hb);
        await this.stockEntryRepository.delete(id);
    }

	async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/stockEntrys.html", "utf8");
