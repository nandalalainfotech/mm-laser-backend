...
export class EmployeeLeaveBalanceService {
    constructor(@InjectRepository(Empleavebalance001mb) private readonly employeeLeaveBalanceRepository: Repository<Empleavebalance001mb>,
        @InjectRepository(Empleavebalance001hb) private readonly employeeLeaveBalancehbRepository: Repository<Empleavebalance001hb>) { }
    ...
    async remove(id: string): Promise<void> {
        let employeeLeaveBalance = this.employeeLeaveBalanceRepository.findOne(id);
        let empleavebalance001hb = new Empleavebalance001hb();
        empleavebalance001hb.setProperties((await employeeLeaveBalance));
        this.employeeLeaveBalancehbRepository.save(empleavebalance001hb);
        await this.employeeLeaveBalanceRepository.delete(id);
    }

    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/employeeLeaveBalance.html", "utf8");
