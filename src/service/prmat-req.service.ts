...
export class PrmatReqService {
	constructor(@InjectRepository(Prmatreq001hb) private readonly prmatReqthbRepository: Repository<Prmatreq001hb>,
                @InjectRepository(Prmatreq001mb) private readonly prmatReqtRepository: Repository<Prmatreq001mb>) { }
    ...
    async remove(id: number): Promise<void> {
        let  prmatreq = this.prmatReqtRepository.findOne(id);
        let  prmatreq001hb = new Prmatreq001hb();
        prmatreq001hb.setProperties((await  prmatreq));
        this.prmatReqthbRepository.save(prmatreq001hb);
        await this.prmatReqtRepository.delete(id);
    }

	async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/prmat-req.html", "utf8");
