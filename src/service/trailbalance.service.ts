...
export class TrailBalanceservice {
    constructor(@InjectRepository(Trialbalance001hb) private readonly trailbalancehbRepository: Repository<Trialbalance001hb>,
    @InjectRepository(Trialbalance001mb) private readonly trailbalanceRepository: Repository<Trialbalance001mb>,) { }
    ...
    async remove(id: number): Promise<void> {
        let unitofMeasure = this.trailbalanceRepository.findOne(id);
        let trialbalance001hb = new Trialbalance001hb();
        trialbalance001hb.setProperties((await unitofMeasure));
        this.trailbalancehbRepository.save(trialbalance001hb);
        await this.trailbalanceRepository.delete(id);
    }

    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/trailbalance.html", "utf8");
