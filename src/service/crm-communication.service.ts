...
export class CrmCommunicationService {
    constructor(@InjectRepository(Communication001mb) private readonly crmCommunicationRepository: Repository<Communication001mb>,
        @InjectRepository(Communication001hb) private readonly crmCommunicationhbRepository: Repository<Communication001hb>) { }
    ...
    async remove(id: number): Promise<void> {
        let Communication = this.crmCommunicationRepository.findOne(id);
        let communication001hb = new Communication001hb();
        communication001hb.setProperties((await Communication));
        this.crmCommunicationhbRepository.save(communication001hb);
        await this.crmCommunicationRepository.delete(id);
    }

    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/crmCommunication.html", "utf8");
