...
export class BudgetMonthDistService {
    ...
    constructor(@InjectRepository(Budgetmonthdist001mb) private readonly costCenterRepository: Repository<Budgetmonthdist001mb>,
        @InjectRepository(Budgetmonthdist001hb) private readonly costCenterhbRepository: Repository<Budgetmonthdist001hb>) { }
    ...
    async remove(id: string): Promise<void> {
        let Budgetmonthdist = this.costCenterRepository.findOne(id);
        let budgetmonthdist001hb = new Budgetmonthdist001hb();
        budgetmonthdist001hb.setProperties((await Budgetmonthdist));
        this.costCenterhbRepository.save(budgetmonthdist001hb);
        await this.costCenterRepository.delete(id);
    }

    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/budgetMonthDist.html", "utf8");
