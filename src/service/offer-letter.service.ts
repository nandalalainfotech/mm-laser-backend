...
export class OfferLetterService {
  constructor(
    @InjectRepository(Recruitoffer001mb)
    private readonly offerLetterRepository: Repository<Recruitoffer001mb>,
    @InjectRepository(Recruitoffer001hb)
    private readonly offerLetterhbRepository: Repository<Recruitoffer001hb>
  ) {}
  ...
  async update(recruitofferDTO: RecruitofferDTO): Promise<Recruitoffer001mb> {
    const recruitoffer001mb = new Recruitoffer001mb();
    recruitoffer001mb.setProperties(recruitofferDTO);
    await this.offerLetterRepository.update(
      { offerId: recruitoffer001mb.offerId },
      recruitoffer001mb
    );
  ...
  async remove(id: number): Promise<void> {
    let offerLetter = this.offerLetterRepository.findOne(id);
    let recruitoffer001hb = new Recruitoffer001hb();
    recruitoffer001hb.setProperties(await offerLetter);
    this.offerLetterhbRepository.save(recruitoffer001hb);
    await this.offerLetterRepository.delete(id);
  }
  async downloadPdf(@Req() request: Request, @Res() response: Response) {
    var html = fs.readFileSync("./html_pdf/offerLetter.html", "utf8");
    ...
    if (offerLetter.length < 0) {
      ...
      for (let i = 0; i < offerLetter.length; i++) {
        ...
        worksheet.getCell("B" + temp).value =
          offerLetter[i].applicantNamePrefix;
