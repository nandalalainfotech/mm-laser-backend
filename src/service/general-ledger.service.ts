...
export class GeneralLedgerService {
  constructor(
    @InjectRepository(Accgenledge001mb)
    private readonly generalledgerRepository: Repository<Accgenledge001mb>,
    @InjectRepository(Accgenledge001hb)
    private readonly generalledgerhbRepository: Repository<Accgenledge001hb>
  ) {}
  ...
  async update(accGenledgeDTO: AccgenledgeDTO): Promise<Accgenledge001mb> {
    const accgenledge001mb = new Accgenledge001mb();
    accgenledge001mb.setProperties(accGenledgeDTO);
    await this.generalledgerRepository.update(
      { accglId: accgenledge001mb.accglId },
      accgenledge001mb
    );
  ...
  async remove(id: string): Promise<void> {
    let generalledger = this.generalledgerRepository.findOne(id);
    let accgenledge001hb = new Accgenledge001hb();
    accgenledge001hb.setProperties(await generalledger);
    this.generalledgerhbRepository.save(accgenledge001hb);
    await this.generalledgerRepository.delete(id);
  }

  async downloadPdf(@Req() request: Request, @Res() response: Response) {
    var html = fs.readFileSync("./html_pdf/generalledger.html", "utf8");
  ...
  async downloadExcel(@Req() request: Request, @Res() response: Response) {
    ...
    if (generalledger.length < 0) {
      ...
      worksheet.getCell("A1:T1").alignment = {
        vertical: "middle",
        horizontal: "center",
      };
      ...
      worksheet.getCell("A2:T2").alignment = {
        vertical: "middle",
        horizontal: "center",
      };
