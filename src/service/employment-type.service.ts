...
export class EmploymentTypeService {
    constructor(@InjectRepository(Employmenttype001mb) private readonly employmenttypeRepository: Repository<Employmenttype001mb>,
        @InjectRepository(Employmenttype001hb) private readonly employmenttypehbRepository: Repository<Employmenttype001hb>) { }
    ...
    async remove(id: number): Promise<void> {
        let Employmenttype = this.employmenttypeRepository.findOne(id);
        let employmenttype001hb = new Employmenttype001hb();
        employmenttype001hb.setProperties((await Employmenttype));
        this.employmenttypehbRepository.save(employmenttype001hb);
        await this.employmenttypeRepository.delete(id);
    }

    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/employmenttype.html", "utf8");
