...
export class EmployeeHolidayWorkService {
    constructor(@InjectRepository(Empworkonholiday001mb) private readonly employeeHolidayWorkRepository: Repository<Empworkonholiday001mb>,
        @InjectRepository(Empworkonholiday001hb) private readonly employeeHolidayWorkhbRepository: Repository<Empworkonholiday001hb>) { }
    ...
    async remove(id: number): Promise<void> {
        let employeeHolidayWork = this.employeeHolidayWorkRepository.findOne(id);
        let empworkonholiday001hb = new Empworkonholiday001hb();
        empworkonholiday001hb.setProperties((await employeeHolidayWork));
        this.employeeHolidayWorkhbRepository.save(empworkonholiday001hb);
        await this.employeeHolidayWorkRepository.delete(id);
    }

    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/employeeHolidayWork.html", "utf8");
