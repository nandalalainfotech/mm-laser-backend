...
export class OperationService {
  constructor(
    @InjectRepository(Operationbom001mb)
    private readonly operationRepository: Repository<Operationbom001mb>,
    @InjectRepository(Operationbom001hb)
    private readonly operationhbRepository: Repository<Operationbom001hb>
  ) {}
  ...
  async remove(id: string): Promise<void> {
    let operation = this.operationRepository.findOne(id);
    let operationbom001hb = new Operationbom001hb();
    operationbom001hb.setProperties(await operation);
    this.operationhbRepository.save(operationbom001hb);
    await this.operationRepository.delete(id);
  }

  async downloadPdf(@Req() request: Request, @Res() response: Response) {
    var html = fs.readFileSync("./html_pdf/operation.html", "utf8");
