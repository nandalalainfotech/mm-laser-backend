...
export class CrmContactService {
    ...
    constructor(@InjectRepository(Crmspcontact001mb) private readonly crmContactRepository: Repository<Crmspcontact001mb>,
        @InjectRepository(Crmspcontact001hb) private readonly crmContacthbRepository: Repository<Crmspcontact001hb>) { }
    ...
    async remove(id: string): Promise<void> {
        let Crmspcontact = this.crmContactRepository.findOne(id);
        let crmspcontact001hb = new Crmspcontact001hb();
        crmspcontact001hb.setProperties((await Crmspcontact));
        this.crmContacthbRepository.save(crmspcontact001hb);
        await this.crmContactRepository.delete(id);
    }

    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/crmContact.html", "utf8");
