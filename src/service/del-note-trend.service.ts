...
export class DelNoteTrendService {
    ...
    constructor(@InjectRepository(Delnotetrends001mb) private readonly delNoteRepository: Repository<Delnotetrends001mb>,
        @InjectRepository(Delnotetrends001hb) private readonly delNotehbRepository: Repository<Delnotetrends001hb>) { }
    ...
    async remove(id: string): Promise<void> {
        let delNote = this.delNoteRepository.findOne(id);
        let delnotetrends001hb = new Delnotetrends001hb();
        delnotetrends001hb.setProperties((await delNote));
        this.delNotehbRepository.save(delnotetrends001hb);
        await this.delNoteRepository.delete(id);
    }

    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/delNote.html", "utf8");
