...
export class RequestItemTransferService {
	constructor(@InjectRepository(Reqitemtransfer001hb) private readonly requestItemTransferhbRepository: Repository<Reqitemtransfer001hb>,
                @InjectRepository(Reqitemtransfer001mb) private readonly requestItemTransferRepository: Repository<Reqitemtransfer001mb>) { }
	...
	async remove(id: number): Promise<void> {
        let reqitemtransfer = this.requestItemTransferRepository.findOne(id);
        let reqitemtransfer001hb = new Reqitemtransfer001hb();
        reqitemtransfer001hb.setProperties((await reqitemtransfer));
        this.requestItemTransferhbRepository.save(reqitemtransfer001hb);
        await this.requestItemTransferRepository.delete(id);
    }

	async downloadPdf(@Req() request: Request, @Res() response: Response) {
        var html = fs.readFileSync("./html_pdf/requestItemTransfer.html", "utf8");
