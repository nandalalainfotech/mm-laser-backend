...
export class ProdStockService {
	constructor(@InjectRepository(Prodstockentry001hb) private readonly prodstockhbRepository: Repository<Prodstockentry001hb>,
				@InjectRepository(Prodstockentry001mb) private readonly prodstockRepository: Repository<Prodstockentry001mb>,) { }
	...
	async remove(id: number): Promise<void> {
        let prodstockentry = this.prodstockRepository.findOne(id);
        let prodstockentry001hb = new Prodstockentry001hb();
        prodstockentry001hb.setProperties((await prodstockentry));
        this.prodstockhbRepository.save(prodstockentry001hb);
        await this.prodstockRepository.delete(id);
    }

	async downloadPdf(@Req() request: Request, @Res() response: Response) {
		var html = fs.readFileSync("./html_pdf/prodStock.html", "utf8");
